...
def bucket_mixed_data(dce = False):
    def fit_bins(values,num_quantiles):
        qt = KBinsDiscretizer(n_bins = num_quantiles,encode = 'ordinal', strategy = 'quantile')
        qt = qt.fit(values)
        return qt
    ...
    make_categorical =  ['ExternalRiskEstimate', 'MSinceOldestTradeOpen', 'MSinceMostRecentTradeOpen','AverageMInFile',
...
def bin_columns(df,number_of_bins,columns,binner = None):
    ...
    for col in columns:
        if new_bin:
            bin = KBinsDiscretizer(n_bins=number_of_bins,encode=encoder,strategy = strategy)
...
def process_lending():
    ...
    url = path+file_name
    df = pd.read_csv(url)
    selected_columns =  ['loan_status','emp_length','home_ownership','annual_inc','addr_state','open_acc','purpose','grade','earliest_cr_line','loan_amnt' ,'term']#loan_amnt & termadded JF 2/8/20
    X_columns = ['emp_length','home_ownership','annual_inc','open_acc','purpose','grade','loan_amnt','term']#'earliest_cr_line' assumed to be credit history may be 'mths_since_last_delinq' instead alos removed 'years_of_credit','addr_state',
    y_column = 'loan_status'
    class_name = y_column
    non_continuous = ['home_ownership','purpose','grade' , 'term']#removed 'addr_state',
    continuous = ['emp_length','annual_inc','open_acc','loan_amnt' ]#removed 'years_of_credit',
    df = df[selected_columns]#not all columns wanted
    df = df.dropna(axis=0)
    df = df.reset_index()
    years_of_credit = list()
    for i in range(df.shape[0]):#change dates from month-year to number of years before  September 2011
    ...
    for i in range(df.shape[0]):
        ...
        if value == '< 1 year':
            ...
            out = 10
        if out == 11:
            print('unaccounted for value ',value,' in determining employment length')
            breakpoint()
    ...
    for i in range(len(df['loan_status'])):
        if df['loan_status'][i] == ('Fully Paid' or 'Does not meet the credit policy. Status:Fully Paid'):
    ...
    def get_df_dce(df_in):
        ...
        #df_dce.drop(columns = 'index',inplace=True)
        return df_dce
    df_dce = get_df_dce(df)#need a unenencoded/scaled dataset for dcf and dce moved below loan status to int
    ...
    type_features, features_type = prepare_dataset.recognize_features_type(df, class_name)
    idx_features = {i: col for i, col in enumerate(X_columns)}
    dummy = {}
    scalers = {}
    mads = {} #for use with lime as a fiddle factor to allow lime coeffs to compare continuous to non_continuous , add dict of continuous variables {variable : mad}
    for column in df_X.columns:
        if column not in non_continuous:#for continuous columns
            ...
            #use get dummies
            dummy_columns = pd.get_dummies(df_X[column],prefix=column, prefix_sep='_',drop_first= False)
...
def get_lending_human_dictionary():
    ...
    d['home_ownership_MORTGAGE'] = {'name':'home ownership','value':'mortgaged'}
    ...
    d['purpose_credit_card'] = {'name': 'purpose of loan','value':'credit card'}
    d['purpose_debt_consolidation'] = {'name': 'purpose of loan','value':'debt consolidation'}
    d['purpose_educational'] = {'name': 'purpose of loan','value':'educational'}
    d['purpose_home_improvement'] = {'name': 'purpose of loan','value':'home improvement'}
    d['purpose_house'] = {'name': 'purpose of loan','value':'house'}
    d['purpose_major_purchase'] = {'name': 'purpose of loan','value':'major purchase'}
    ...
    d['purpose_renewable_energy'] = {'name': 'purpose of loan','value':'renewable energy'}
    d['purpose_small_business'] = {'name': 'purpose of loan','value':'small business'}
...
def process_fico_data():#replaces process_cat_dataset with what worked in data_exploration/data_exploration.py
    ...
    dce = True
    if (dce == True):
        df, is_categorical, make_categorical, binner, df_dce = bucket_mixed_data(dce)
    else:
    ...
    df_encoded,possible_outcomes,labels = get_possible_outcomes(df_encoded,class_name)
    invariants = ''
    scaler = ''
...
def process_cat_dataset():
    ...
    df_encoded, label_encoder = label_encode(df_bin, make_categorical+is_categorical)
...
def process_adult():
    path = 'datasets/'
    file_name = 'adult.csv'#'vate '
    url = path+file_name
    df = pd.read_csv(url)
    selected_columns =  ['work_class','age','education_years','marital_status','occupation','race','sex','hours_per_week','income']
    X_columns = ['work_class','age','education_years','marital_status','occupation','race','sex','hours_per_week']
    y_column = 'income'
    class_name = y_column
    non_continuous = ['work_class','marital_status','occupation','race','sex']
    continuous = ['age', 'education_years','hours_per_week']

    #df = df[selected_columns]#not all columns wanted
    df = df.dropna(axis=0)
    df = df.reset_index()
    #education years needs to be made from mixed data


    df = df.drop(labels=['index','fnlwgt','education','relationship','capital_gain','capital_loss','native_country'],axis=1)
    ...
    target = df[class_name]
    """
    target = np.empty(df[class_name].shape)
    for i in range(len(df[class_name])):
        if df[class_name][i] == (possible_outcomes[1]):
            target[i] = 1
        elif df[class_name][i] == (possible_outcomes[0]):
            target[i] = 0
        else:
            print('incorrect target value in y variable')
            breakpoint()
    """
    for column in df_X.columns:
    ...
    dataset = {
        ...
        'labels' : labels,
        'invariants' : invariants,
        'data_human_dict' :{'income':'income',
                            'age':'age',
                            'work_class_ ?': {'name':'work_class','value':'?'},
                            'work_class_ Federal-gov':{'name':'work_class','value':'Federal-gov'},
                            'work_class_ Local-gov':{'name':'work_class','value':'Local-gov'},
                            'work_class_ Never-worked':{'name':'work_class','value':'Never-worked'},
                            'work_class_ Private':{'name':'work_class','value':'Private'},
                            'work_class_ Self-emp-inc':{'name':'work_class','value':'Self-emp-inc'},
                            'work_class_ Self-emp-not-inc':{'name':'work_class','value':'Self-emp-not-inc'},
                            'work_class_ State-gov':{'name':'work_class','value':'State-gov'},
                            'work_class_ Without-pay':{'name':'work_class','value':'Without-pay'},
                            'education_years':'education_years',
                            'marital_status_ Divorced':{'name':'marital_status','value':'Divorced'},
                            'marital_status_ Married-AF-spouse':{'name':'marital_status','value':'Married-AF-spouse'},
                            'marital_status_ Married-civ-spouse':{'name':'marital_status','value':'Married-civ-spouse'},
                            'marital_status_ Married-spouse-absent':{'name':'marital_status','value':'Married-spouse-absent'},
                            'marital_status_ Never-married':{'name':'marital_status','value':'Never-married'},
                            'marital_status_ Separated':{'name':'marital_status','value':'Separated'},
                            'marital_status_ Widowed':{'name':'marital_status','value':'Widowed'},
                            'occupation_ ?':{'name':'occupation','value':'?'},
                            'occupation_ Adm-clerical':{'name':'occupation','value':'Adm-clerical'},
                            'occupation_ Armed-Forces':{'name':'occupation','value':'Armed-Forces'},
                            'occupation_ Craft-repair':{'name':'occupation','value':'Craft-repair'},
                            'occupation_ Exec-managerial':{'name':'occupation','value':'Exec-managerial'},
                            'occupation_ Farming-fishing':{'name':'occupation','value':'Farming-fishing'},
                            'occupation_ Handlers-cleaners':{'name':'occupation','value':'Handlers-cleaners'},
                            'occupation_ Machine-op-inspct':{'name':'occupation','value':'Machine-op-inspct'},
                            'occupation_ Other-service':{'name':'occupation','value':'Other-service'},
                            'occupation_ Priv-house-serv':{'name':'occupation','value':'Priv-house-serv'},
                            'occupation_ Prof-specialty':{'name':'occupation','value':'Prof-specialty'},
                            'occupation_ Protective-serv':{'name':'occupation','value':'Protective-serv'},
                            'occupation_ Sales':{'name':'occupation','value':'Sales'},
                            'occupation_ Tech-support':{'name':'occupation','value':'Tech-support'},
                            'occupation_ Transport-moving':{'name':'occupation','value':'Transport-moving'},
                            'race_ Amer-Indian-Eskimo':{'name':'race','value':'Amer-Indian-Eskimo'},
                            'race_ Asian-Pac-Islander':{'name':'race','value':'Asian-Pac-Islander'},
    ...
    pickle_filename = 'pickled_data/adult_pickled_data_MinMax_4_07_22.p'
    outfile = open(pickle_filename,'wb')
    pickle.dump(dataset,outfile)
    outfile.close()
...
def process_compas():
    ...
    selected_columns =  ['sex','age', 'race','c_charge_degree',  'priors_count','score_text']
    X_columns = ['sex','age', 'race','c_charge_degree',  'priors_count']
    y_column = 'score_text'#needs transforming to 0 for not High and 1 for High

    class_name = y_column
    non_continuous = ['sex', 'race','c_charge_degree']
    continuous = ['age', 'priors_count' ]

    df = df.drop(labels=['id', 'name', 'first', 'last', 'compas_screening_date', 'dob',
        'age_cat', 'juv_fel_count', 'juv_misd_count',  'decile_score','juv_other_count',
    ...
    dataset = {
        ...
        'labels' : labels,
        'invariants' : invariants,
        'data_human_dict' :{'age':'age',
                            'race_African-American':{'name':'race','value':'African-American'},
                            'race_Asian':{'name':'race','value':'Asian'},
                            'race_Caucasian':{'name':'race','value':'Caucasian'},
                            'race_Hispanic':{'name':'race','value':'Hispanic'},
                            'race_Native American':{'name':'race','value':'Native American'},
