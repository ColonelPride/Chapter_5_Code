...
def label_decode_jf(df, columns, label_encoder):#changed to allow for not all discrete columns being label encoded
    df_de = df.copy(deep=True)
    for col in columns:
...
def build_df2explain(bb, X, dataset):
    ...
    """
    dfZ = pd.DataFrame(data=yX, columns=columns)
    #no decoding here JF 11/11/2019
    #dfZ = label_decode(dfZ, discrete, label_encoder)
    #dfZ = label_decode_jf(dfZ, discrete, label_encoder)#changed by jf
    return dfZ
