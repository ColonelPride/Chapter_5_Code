...
def new_cell(parameter_dict, dataset, model, prediction, target_vals):
    ...
    while flag:  # while combined distance == 0 flag is true , flag to false  i.e. must be different to reference cell
        ...
        if flag == False:
            if combined_distance(parameter_dict, target_vals, line[0], dataset) == 0:
        ...
        if count > limit:
            ...
            cell = None
            return cell
    dist_to_target = combined_distance(parameter_dict, target_vals, line[0], dataset)
    cell = {'value': line,
            'norm_cost': 0,
            'cost': cost,
            # 'norm_distance':0,
...
def create_mutated_cell(parameter_dict,value, norm_cost, model, prediction, dataset, target_vals):  # value is an array
    ...
    dist_to_target = combined_distance(parameter_dict,target_vals, value[0], dataset)  # 'fault here some distances of clones == 0'
    cell = {'value': value,
            'norm_cost': norm_cost,
            'cost': cost,
            'distance': dist_to_target}
...
def mutate(parameter_dict, child, normalized_cost, dataset, model, prediction):  # child is a 2d array [i][v]
    count = 0
    ...
    while flag:
        ...
        for i in range(len(columns)):  # -1 because class name not in line
            ...
            if columns[i] in dataset['non_continuous']:
                if columns[i] not in dataset['invariants']:
                    ...
                    if r_alpha < 10 * parameter_dict['beta'] * alpha:  # 10*alpha to makea mutation more likely
                        value = select_random_value(columns[i], dataset)
                        if columns[i] in dummy.keys():
                            for dum in dummy[columns[i]]:
                                if dum == value:
        ...
        if flag == False:  # check distance if 0 not allowed why is cost constraint allowing this?
            if combined_distance(parameter_dict, child['value'][0], df_line.values, dataset) == 0:
                flag = True
                print('error: distance between target cell and mutated cell is 0')
                breakpoint()
        count = count + 1
        if count > limit:
            return child['value']
...
def clone_cell(parameter_dict, parent,  dataset, model, prediction, target_vals):
    clones = list()
    ...
    for i in range(parameter_dict['num_clones']):
        v = mutate(parameter_dict, parent, normalized_cost, dataset, model, prediction)
        child = create_mutated_cell(parameter_dict, v, normalized_cost, model, prediction, dataset, target_vals)
        if child['distance'] == 0:
            print('child == fact')
            breakpoint()
        clones.append(child)
...
def get_neighborhood(parameter_dict,reference_cell, pop, aff_thresh, dataset):
    ...
    for cell in pop:
        ...
        if combined_distance(parameter_dict, reference_cell['value'][0], cell['value'][0], dataset) < aff_thresh:
...
def affinity_suppress(parameter_dict,population, aff_thresh, dataset):
    ...
    while flag:
        new_list = list()
        first = population[0]
        neighbors, population = get_neighborhood(parameter_dict, first, population, aff_thresh,
...
def affinity_suppress_by_cost(parameter_dict, population, aff_thresh, dataset):
    ...
    while flag:
        new_list = list()
        first = population[0]
        neighbors = get_neighborhood(parameter_dict, first, population, aff_thresh, dataset)
...
def search( parameter_dict, aff_thresh, dataset, model, prediction, target_vals ):
    print('begin search')
    ...
    for i in range(parameter_dict['pop_size']):
        # print('get new cell')
        cell = new_cell(parameter_dict, dataset, model, prediction, target_vals)
        if cell != None:
            pop.append(cell)
    if pop == []:
    ...
    for g in range(parameter_dict['max_gens']):
        ...
        while flag:
            progeny = list()
            for p in pop:
                cell = clone_cell(parameter_dict, p, dataset, model, prediction, target_vals)
                progeny.append(cell)
            if parameter_dict['sort_by'] == 'distance':
        ...
        if num_of_att > 1:
            sorted_progeny = affinity_suppress(parameter_dict,progeny, aff_thresh, dataset)
            if parameter_dict['sort_by'] == 'distance':
            ...
            if columns[0] in dataset['non_continuous']:
                sorted_progeny = affinity_suppress_by_cost(parameter_dict, progeny, aff_thresh, dataset)
                sorted_progeny = sort_by_cost(sorted_progeny)
            else:
                sorted_progeny = affinity_suppress(parameter_dict, progeny, aff_thresh, dataset)
                sorted_progeny = sort_by_dist(sorted_progeny)
        ...
        stop_cost = stop_cost / (parameter_dict['problem_size'] * len(sorted_progeny))
        print('stop_cost: ', stop_cost, 'stop_cost/problem_size: ', stop_cost / (parameter_dict['problem_size'] * len(sorted_progeny)))
        ...
        if stop_cost < parameter_dict['stop_condition']:
            ...
            break
        if g < parameter_dict['max_gens'] - 2:  # (max_gens/2 -1)this inserts random cells in evry generation, other term stops adding cells halfway through and just optimises no cells added in last generation
            new_cell_num = int(parameter_dict['pop_size'] * parameter_dict['new_cell_rate'])  # get initial population from parameter dict

            for i in range(new_cell_num):  # get from global

                if len(sorted_progeny) < parameter_dict['pop_size']:  # get initial pop from global
                    sorted_progeny.append(new_cell(parameter_dict, dataset, model, prediction, target_vals))
...
def create_optAINet_explanation(target_vals, y, pop, dataset, prob_dict, record_number, db_file, imp_arr):
    ...
    with(conn):
        ...
        class_value = str(y)
        operand = '='
        attributes_id = write_to_db.db_add_attributes_bundle(conn, class_name, operand, y)
        ...
        if dataset['use_dummies'] == True:
            ...
            for col in dataset['X_columns']:
                ...
                att_value = write_to_db.decode_debin_db(att_type, att_value, dataset)
            ...
            # add importances
            importance_id = write_to_db.db_add_importances(conn, explanations_id)
            for i in range(len(imp_arr)):
                line_columns = get_line_columns(dataset)
                att_type = line_columns[i]
                operand = '='
                att_value = imp_arr[i]
                attribute_id = write_to_db.db_add_attributes_bundle(conn, att_type, operand, att_value)
                importances_attributes_id = write_to_db.db_add_importances_attributes(conn, importance_id, attribute_id)
        else:
            for col in dataset['X_columns']:
                att_type = col
                operand = '='
                ...
                if col in line_columns:
                    if dataset['use_dummies'] == True:
                        if col in dataset['continuous']:
                            ...
                            att_value = write_to_db.decode_debin_db(col, att_value, dataset, decode=True)
                            att_name = dataset['data_human_dict'][col]
                            attributes_id = write_to_db.db_add_attributes_bundle(conn, att_name, operand, att_value)
                            attributes_deltas_id = write_to_db.db_add_deltas_attributes(conn, deltas_id, attributes_id)
        ...
        for key in prob_dict:
            ...
            class_probabilities_attributes_id = write_to_db.db_add_class_probabilities_attributes(conn, class_probabilities_id,
                                                                                      attributes_id)
...
def init_var_optAINet(blackbox, X, line_number, dataset, prob_dict, db_file, imp_arr, df_out,parameter_dict):
    def predict_label(y):
        if y < 0.5:
            return 0
        else:
    ...
    aff_thresh = ((parameter_dict['search_space'][1] - parameter_dict['search_space'][0]) / parameter_dict['affinity_constant']) / (
            1 + len(dataset['X_columns']) - len(dataset['invariants']))
    best, df_pop = search(parameter_dict, aff_thresh,dataset, blackbox, prediction, line)
