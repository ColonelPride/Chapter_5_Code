...
def debin_db(k, val_string, dataset):
    ...
    for i in range(len(binner[k].bin_edges_[0])) :
        if binner[k].bin_edges_[0][i] <= v:#what if in last bucket? how to get upper bound?
            count = i
        else:
            break
    ...
    if count< dataset['number_of_bins']:
        ...
        if binner[k].bin_edges_[0][count] == binner[k].bin_edges_[0][count+1]-1:
            val_string = str(binner[k].bin_edges_[0][count])
        else:
            val_string = 'between '+str(binner[k].bin_edges_[0][count]) + ' and ' + str((binner[k].bin_edges_[0][count+1])-1)
    else:
        val_string = 'greater than or equal to '+str(binner[k].bin_edges_[0][count])
    return val_string
...
def bin_db(k,val_string,dataset):
    ...
    if k in dataset['binner']:
        ...
        if value >= 0:#all special values <0
            ...
            #bin values
            value = dataset['binner'][k].transform([[value]])#value mustr be 2D array
...
def decode_debin_db(k,val_string,dataset,decode=True):
    ...
    if k in dataset['scaler']:
        val_arr = descale_db(k, val_arr, dataset)#val_arr is a value not an array
    return val_arr
...
def db_add_attributes_operands_types(conn,attribute_type_id,operand_id):
    sql = '''INSERT INTO attributes_operands_types (attribute_type_id,operand_id)
            VALUES(?,?)   '''
    cur = conn.cursor()
    cur.execute(sql,(attribute_type_id,operand_id,) )
    return cur.lastrowid
...
def db_add_attributes(conn,attributes_operands_types_id,attribute_value):
    sql = '''INSERT INTO attributes (attributes_operands_types_id,attribute_value)
            VALUES(?,?)   '''
    cur = conn.cursor()
    cur.execute(sql,(attributes_operands_types_id, attribute_value,) )
    return cur.lastrowid
...
def db_add_records_attributes(conn,attributes_id,records_id):
    sql = '''INSERT INTO records_attributes(attribute_id,record_id)
            VALUES(?,?)   '''
    cur = conn.cursor()
    cur.execute(sql,(attributes_id,records_id,) )
    return cur.lastrowid
...
def db_add_rules_attributes(conn,rules_id,attributes_id ):
    sql = '''INSERT INTO rules_attributes(rule_id,attribute_id)
            VALUES(?,?)   '''
    cur = conn.cursor()
    cur.execute(sql,(rules_id,attributes_id,) )
    return cur.lastrowid
...
def db_add_deltas_attributes(conn,deltas_id,attributes_id ):
    sql = '''INSERT INTO attributes_deltas(delta_id,attribute_id)
            VALUES(?,?)   '''
    cur = conn.cursor()
    cur.execute(sql,(deltas_id,attributes_id,) )
    return cur.lastrowid
...
def db_add_importances_attributes(conn,importances_id,attributes_id ):
    sql = '''INSERT INTO importances_attributes(importance_id,attribute_id)
            VALUES(?,?)   '''
    cur = conn.cursor()
    cur.execute(sql,(importances_id,attributes_id,) )
    return cur.lastrowid
...
def db_add_class_probabilities_attributes(conn,class_probability_id,attributes_id ):
    sql = '''INSERT INTO class_probabilities_attributes(class_probability_id,attribute_id)
            VALUES(?,?)   '''
    cur = conn.cursor()
    cur.execute(sql,(class_probability_id,attributes_id,) )
    return cur.lastrowid
...
def db_add_attributes_bundle(conn, att_type, operand, att_val):
    ...
    attributes_type_id = db_add_attribute_types(conn,att_type)#write human desc to db
    operands_id = db_add_operands(conn,operand)
    attributes_operands_types_id = db_add_attributes_operands_types(conn,attributes_type_id,operands_id)
    attributes_id = db_add_attributes(conn,attributes_operands_types_id, att_val)
...
def seperate_double_value(v,op_chars):
    ...
    att_name = v[(op_indices[0]):(op_indices[1]-2)]#att_name not necessary
