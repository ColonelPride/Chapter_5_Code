...
@author: chrisr
"""
#from Chris Russels Bitbucket @
#implemenatation of  'Efficient Search for Diverse Coherent Explanations'
import numpy as np
from types import SimpleNamespace
import sklearn.linear_model as skl
import gurobipy as gb
import json #added by JF
import pandas as pd
...
class linear_explanation:
    ...
    def train_logistic(self,target,subset=False):
        ...
        the target labels"""
        lr=skl.logistic.LogisticRegressionCV(solver='sag',max_iter=1000)#max_iter changed from default of 100 because not converging
    ...
    def explain(self,cf, labels=("'good'","'bad'")):
        ...
        return explain
    """
    #new function to write corect data structure, change from list of dict to list of dict of list of dict



    def explain(self,cf, counter,labels=("'good'","'bad'")):
        test_entry=self.short_factual
        #direction=np.sign(self.evaluate(self.long_factual)) *-1
        out=cf
        ...
        #outer_dict = {}
        inner_list = list()
        #create empty np array of 23 values called check_arr
        check_arr = np.zeros((mask.shape))
        for i in range(mask.size):
            if mask[i]:
                #check_arr[i] gets cf value
                check_arr[i] = self.pp(out[i],test_entry[i],i).strip()
                inner_dict ={}
                #add conditional that if self.cox[i].name in dataset['categorical']
                name = self.cox[i].name
                inner_dict['att_name'] = name
                inner_dict['user_value'] =  self.pp(test_entry[i],out[i],i).strip()
                inner_dict['cf_value'] = self.pp(out[i],test_entry[i],i).strip()
                inner_list.append(inner_dict)
            else:
                check_arr[i] = self.pp(test_entry[i],out[i],i).strip() #give user value

        #check_text=self.explain_entry(check_arr)#unsurpisingly has huge recursion
        #print('check text: ',check_text)
        return inner_list

    def give_explanation(self,labels,df2explain,idx_record2explain,check_csv,upto=10):#need to add df2explain
        actual_score=labels[int(self.direction>0)]
        cf_score=labels[int(not(self.direction>0))]
        assert(upto>=1)
        counter = 0
        ...
        delta = {}
        delta[str(counter)]= self.explain(out,counter,labels)
        counter = counter + 1
        old_out=out
        ...
        for i in range(upto-1):
            ...
            counter = counter + 1
        explanation ={}
        explanation['score'] = actual_score
        explanation['cf_score'] = cf_score
        explanation['deltas'] = delta
        def write_dce_csv(exp,df2explain,idx_record2explain):
            import csv
            #with open('dce.csv','w',newline='')as csv_file:
                #dce_writer = csv.writer(csv_file,delimiter =';')
                #write csv of columns and values
            #exploit pandas instead
            header = df2explain.columns
            csv_df = pd.DataFrame(columns=header,data=None)
            deltas = exp['deltas']
            for k in deltas.keys():
                row = df2explain.iloc[idx_record2explain].copy(deep=True)
                #print(row.values)
                for i in range (len(deltas[k])):
                    #write value cf_value to column att_name
                    cf_value = deltas[k][i]['cf_value']
                    att_name = deltas[k][i]['att_name']
                    row[att_name]=cf_value
                #print(row.values)
                csv_df = csv_df.append(row)
            csv_df.to_csv('dce_check.csv',sep=';')
        if check_csv:
            write_dce_csv(explanation,df2explain,idx_record2explain)
        return explanation
    """
    ...
    """
    #changed to return JSON for writing database rather than text
    def explain(self,cf, labels=("'good'","'bad'")):
        test_entry=self.short_factual
        #direction=np.sign(self.evaluate(self.long_factual)) *-1
        actual_score=labels[int(self.direction>0)]
        ...
        mask= np.abs(out- test_entry)>0.001
        explain ={}
        explain.update({'score':actual_score})
        explain.update({'cf_score':cf_score})
        values  = {}
        for i in range(mask.size):
            if mask[i]:
                #add conditional that if self.cox[i].name in dataset['categorical']
                ...
                if name in dataset['categorical']:
                    column_le = dataset['label_encoder']
                    #print('att_name: ',att_name)
                    #coded_cf = self.pp(test_entry[i],out[i],i) #changed because was producing user value why I do not understand JF
                    coded_cf = self.pp(out[i],test_entry[i],i)
                    #print('coded_cf: ', coded_cf)
                    column_le = dataset['label_encoder']
                    #cf_value = column_le[name].inverse_transform([int(coded_cf)])
                    #cf_value = {'cf_value': cf_value[0]}
                    cf_value = {'cf_value': coded_cf}
                    #print('cf_value: ',cf_value)
                    #coded_user = self.pp(out[i],test_entry[i],i)#changed because was producing cf value why I do not understand JF
                    coded_user = self.pp(test_entry[i],out[i],i)
                    #print('coded_user: ',coded_user)
                    #user_value = column_le[name].inverse_transform([int(coded_user)])
                    #user_value = {'user_value': user_value[0]}
                    user_value = {'user_value': coded_user}
                    #print('user_value ',user_value)
                else:

                #remove whitepspace with .strip()
                user_value = {'user_value': self.pp(test_entry[i],out[i],i).strip()}
                cf_value = {'cf_value': self.pp(out[i],test_entry[i],i).strip()}
                values.update(att_name)
                values.update(user_value)
                values.update(cf_value)
                explain.update(values)
    ...
    def explain_follow(self,cf, labels=("'Good'","'Bad'")):
        ...
        for i in range(mask.size):
            if mask[i]:
                ...
                """
                user_value = {'user_value': self.pp(test_entry[i],out[i],i).strip()}
                #have a problem with white space in cf_value causing failure to compare string and integer strip whitespace from cf_value here
                cf_value = {'cf_value': self.pp(out[i],test_entry[i],i).strip()}
                values.update(att_name)
                values.update(user_value)
                values.update(cf_value)
                explain.update(values)
        explain_json = json.dumps(explain)
        #no whitespace in cf_value here
        #back to give_explanation
        return explain_json
    ...
    def give_explanation(self,upto=10,labels=("'Good'","'Bad'")):
        ...
        return full_exp
    """
    def explain_entry(self,entry,df2explain,idx_record2explain,check_csv=False,upto=10,labels=("'Good'","'Bad'")):
        self.set_factual(entry)
        self.build_structure()
        text=self.give_explanation(labels,df2explain,idx_record2explain,check_csv,upto=10)
        return text

    def explain_set(self,df2explain,upto):
        entries=df2explain.values
        out=np.empty((entries.shape[0],upto),dtype=np.object)
        for i in range(entries.shape[0]):
            #changed to reflect change in explain_entry
            #tmp=np.hstack(self.explain_entry(entries[i],upto))
            exp = self.explain_entry(entries[i],df2explain,i,upto,False)
            out[i] = exp['score']
        return out
